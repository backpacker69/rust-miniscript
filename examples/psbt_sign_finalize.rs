...
use peercoin::PrivateKey;
use miniscript::peercoin::consensus::encode::deserialize;
use miniscript::peercoin::hashes::hex::FromHex;
use miniscript::peercoin::psbt::PartiallySignedTransaction as Psbt;
use miniscript::peercoin::{
...
fn main() {
    ...
    //let bridge_descriptor = Descriptor::<peercoin::PublicKey>::from_str(&s).expect("parse descriptor string");
    ...
    let spend_tx = Transaction {
        version: 2,
        lock_time: peercoin::absolute::LockTime::from_consensus(5000),
        input: vec![],
        output: vec![],
    };
    ...
    let hash_ty = peercoin::sighash::EcdsaSighashType::All;
    ...
    println!("{}", peercoin::consensus::encode::serialize_hex(&tx));
}
