...
use peercoin::{absolute, ecdsa, secp256k1, Sequence};
...
fn main() {
    ...
    let descriptor = miniscript::Descriptor::<peercoin::PublicKey>::from_str(&s).unwrap();
    ...
    // `peercoin::PublicKey` from the `MiniscriptKey` which can be supplied by
    ...
    let mut sigs = HashMap::<peercoin::PublicKey, ecdsa::Signature>::new();
...
fn spending_transaction() -> peercoin::Transaction {
    peercoin::Transaction {
        version: 2,
        timestamp: 0,
        lock_time: absolute::LockTime::ZERO,
        input: vec![peercoin::TxIn {
            previous_output: Default::default(),
            script_sig: peercoin::ScriptBuf::new(),
            sequence: Sequence::MAX,
            witness: Witness::default(),
        }],
        output: vec![peercoin::TxOut {
            script_pubkey: peercoin::ScriptBuf::new(),
            value: 100_000_000,
        }],
    }
}

fn list_of_three_arbitrary_public_keys() -> Vec<peercoin::PublicKey> {
    #[cfg_attr(feature="cargo-fmt", rustfmt_skip)]
    vec![
        peercoin::PublicKey::from_slice(&[2; 33]).expect("key 1"),
        peercoin::PublicKey::from_slice(&[
            0x02,
