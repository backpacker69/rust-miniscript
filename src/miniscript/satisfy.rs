...
use peercoin::taproot::{ControlBlock, LeafVersion, TapLeafHash};
use peercoin::{absolute, Sequence};
use sync::Arc;
...
pub trait Satisfier<Pk: MiniscriptKey + ToPublicKey> {
    ...
    fn lookup_ecdsa_sig(&self, _: &Pk) -> Option<peercoin::ecdsa::Signature> {
        None
    }
    ...
    /// Given a raw `Pkh`, lookup corresponding [`peercoin::PublicKey`]
    fn lookup_raw_pkh_pk(&self, _: &hash160::Hash) -> Option<peercoin::PublicKey> {
        None
    }

    /// Given a raw `Pkh`, lookup corresponding [`peercoin::secp256k1::XOnlyPublicKey`]
    ...
    ) -> Option<(XOnlyPublicKey, peercoin::taproot::Signature)> {
        None
    }
...
impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk> for HashMap<Pk, peercoin::ecdsa::Signature> {
    fn lookup_ecdsa_sig(&self, key: &Pk) -> Option<peercoin::ecdsa::Signature> {
        self.get(key).copied()
    }
}

impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk>
    for HashMap<(Pk, TapLeafHash), peercoin::taproot::Signature>
{
    fn lookup_tap_leaf_script_sig(
        &self,
        key: &Pk,
...
impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk>
    for HashMap<hash160::Hash, (Pk, peercoin::ecdsa::Signature)>
where
    Pk: MiniscriptKey + ToPublicKey,
{
    ...
    fn lookup_raw_pkh_pk(&self, pk_hash: &hash160::Hash) -> Option<peercoin::PublicKey> {
...
impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk>
    for HashMap<(hash160::Hash, TapLeafHash), (Pk, peercoin::taproot::Signature)>
where
    Pk: MiniscriptKey + ToPublicKey,
{
    fn lookup_tap_leaf_script_sig(
...
impl<'a, Pk: MiniscriptKey + ToPublicKey, S: Satisfier<Pk>> Satisfier<Pk> for &'a S {
    fn lookup_ecdsa_sig(&self, p: &Pk) -> Option<peercoin::ecdsa::Signature> {
...
macro_rules! impl_tuple_satisfier {
    ($($ty:ident),*) => {
            ...
            $($ty: Satisfier< Pk>,)*
        {
            ...
            fn lookup_tap_leaf_script_sig(&self, key: &Pk, h: &TapLeafHash) -> Option<peercoin::taproot::Signature> {
                let &($(ref $ty,)*) = self;
                $(
