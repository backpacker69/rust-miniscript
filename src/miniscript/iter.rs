...
pub mod test {
    ...
    pub type TestData = (
        Miniscript<peercoin::PublicKey, Segwitv0>,
    ...
    pub fn gen_bitcoin_pubkeys(n: usize, compressed: bool) -> Vec<peercoin::PublicKey> {
        gen_secp_pubkeys(n)
            .into_iter()
            .map(|inner| peercoin::PublicKey { inner, compressed })
            .collect()
    }
    ...
    fn find_keys() {
        gen_testcases().into_iter().for_each(|(ms, k, _, _)| {
            assert_eq!(ms.iter_pk().collect::<Vec<peercoin::PublicKey>>(), k);
        })
    }
}
