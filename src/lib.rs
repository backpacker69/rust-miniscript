...
//! use std::str::FromStr;
//!
//! let desc = miniscript::Descriptor::<peercoin::PublicKey>::from_str("\
//!     sh(wsh(or_d(\
...
//! assert_eq!(
//!     desc.address(peercoin::Network::Peercoin).unwrap().to_string(),
...
use peercoin::locktime::absolute;
...
pub trait MiniscriptKey: Clone + Eq + Ord + fmt::Debug + fmt::Display + hash::Hash {
    ...
    /// The associated [`peercoin::hashes::sha256::Hash`] for this [`MiniscriptKey`], used in the
    /// sha256 fragment.
    ...
    /// The associated [`peercoin::hashes::ripemd160::Hash`] for this [`MiniscriptKey`] type, used
    /// in the ripemd160 fragment.
    type Ripemd160: Clone + Eq + Ord + fmt::Display + fmt::Debug + hash::Hash;

    /// The associated [`peercoin::hashes::hash160::Hash`] for this [`MiniscriptKey`] type, used in
    /// the hash160 fragment.
    type Hash160: Clone + Eq + Ord + fmt::Display + fmt::Debug + hash::Hash;
}

impl MiniscriptKey for peercoin::secp256k1::PublicKey {
...
impl MiniscriptKey for peercoin::PublicKey {
...
impl MiniscriptKey for peercoin::secp256k1::XOnlyPublicKey {
    type Sha256 = sha256::Hash;
    type Hash256 = hash256::Hash;
    type Ripemd160 = ripemd160::Hash;
    type Hash160 = hash160::Hash;
...
pub trait ToPublicKey: MiniscriptKey {
    ...
    fn to_public_key(&self) -> peercoin::PublicKey;

    /// Convert an object to x-only pubkey
    fn to_x_only_pubkey(&self) -> peercoin::secp256k1::XOnlyPublicKey {
        let pk = self.to_public_key();
        peercoin::secp256k1::XOnlyPublicKey::from(pk.inner)
    }
...
impl ToPublicKey for peercoin::PublicKey {
    fn to_public_key(&self) -> peercoin::PublicKey {
        *self
    }
...
impl ToPublicKey for peercoin::secp256k1::PublicKey {
    fn to_public_key(&self) -> peercoin::PublicKey {
        peercoin::PublicKey::new(*self)
    }
...
impl ToPublicKey for peercoin::secp256k1::XOnlyPublicKey {
    fn to_public_key(&self) -> peercoin::PublicKey {
        ...
        peercoin::PublicKey::from_slice(&data)
...
pub enum Error {
    ...
    AddrError(peercoin::address::Error),
...
#[doc(hidden)]
impl From<peercoin::secp256k1::Error> for Error {
    fn from(e: peercoin::secp256k1::Error) -> Error {
        Error::Secp(e)
    }
}

#[doc(hidden)]
impl From<peercoin::address::Error> for Error {
    fn from(e: peercoin::address::Error) -> Error {
        Error::AddrError(e)
    }
}
...
#[cfg(test)]
fn hex_script(s: &str) -> peercoin::ScriptBuf {
    let v: Vec<u8> = peercoin::hashes::hex::FromHex::from_hex(s).unwrap();
    peercoin::ScriptBuf::from(v)
}
...
mod tests {
    ...
    fn regression_secp256k1_key_hash() {
        use peercoin::secp256k1::PublicKey;
    ...
    fn regression_xonly_key_hash() {
        use peercoin::secp256k1::XOnlyPublicKey;
