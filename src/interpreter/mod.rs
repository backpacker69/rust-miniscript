...
use peercoin::hashes::{hash160, ripemd160, sha256, Hash};
use peercoin::{absolute, secp256k1, sighash, taproot, Sequence, TxOut, Witness};
...
pub struct Interpreter<'txin> {
    ...
    script_code: Option<peercoin::ScriptBuf>,
    age: Sequence,
...
pub enum KeySigPair {
    ...
    Ecdsa(peercoin::PublicKey, peercoin::ecdsa::Signature),
    /// A x-only key and corresponding Schnorr signature
    Schnorr(peercoin::key::XOnlyPublicKey, peercoin::taproot::Signature),
}

impl KeySigPair {
    /// Obtain a pair of ([`peercoin::PublicKey`], [`peercoin::ecdsa::Signature`]) from [`KeySigPair`]
    pub fn as_ecdsa(&self) -> Option<(peercoin::PublicKey, peercoin::ecdsa::Signature)> {
        match self {
    ...
    /// Obtain a pair of ([`peercoin::secp256k1::XOnlyPublicKey`], [`peercoin::taproot::Signature`]) from [`KeySigPair`]
    pub fn as_schnorr(
        &self,
    ) -> Option<(peercoin::key::XOnlyPublicKey, peercoin::taproot::Signature)> {
        match self {
...
enum BitcoinKey {
    ...
    // Xonly key
    XOnlyPublicKey(peercoin::key::XOnlyPublicKey),
}
...
impl From<peercoin::PublicKey> for BitcoinKey {
    fn from(pk: peercoin::PublicKey) -> Self {
        BitcoinKey::Fullkey(pk)
    }
}

impl From<peercoin::key::XOnlyPublicKey> for BitcoinKey {
    fn from(xpk: peercoin::key::XOnlyPublicKey) -> Self {
...
impl<'txin> Interpreter<'txin> {
    ...
    pub fn verify_sig<C: secp256k1::Verification, T: Borrow<TxOut>>(
        ...
        let mut cache = peercoin::sighash::SighashCache::new(tx);
        match sig {
    ...
    /// since it cannot distinguish between sorted and unsorted multisigs (and anyway
    /// it can only see the final keys, keyorigin info is lost in serializing to Bitcoin).
    /// x-only keys are translated to [`peercoin::PublicKey`] with 0x02 prefix.
    pub fn inferred_descriptor(&self) -> Result<Descriptor<peercoin::PublicKey>, crate::Error> {
...
fn verify_sersig<'txin>(
    ...
    match pk {
        BitcoinKey::Fullkey(pk) => {
            let ecdsa_sig = peercoin::ecdsa::Signature::from_slice(sigser)?;
        ...
        BitcoinKey::XOnlyPublicKey(x_only_pk) => {
            let schnorr_sig = peercoin::taproot::Signature::from_slice(sigser)?;
...
mod tests {
    ...
    fn setup_keys_sigs(
        ...
        Vec<peercoin::PublicKey>,
        Vec<Vec<u8>>,
        Vec<peercoin::ecdsa::Signature>,
        secp256k1::Message,
        Secp256k1<secp256k1::All>,
        Vec<peercoin::key::XOnlyPublicKey>,
        Vec<peercoin::taproot::Signature>,
        Vec<Vec<u8>>,
    ) {
        let secp = secp256k1::Secp256k1::new();
        let msg = secp256k1::Message::from_slice(&b"Yoda: ppc, I trust. HODL I must!"[..])
            .expect("32 bytes");
        let mut pks = vec![];
        ...
        for i in 1..n + 1 {
            ...
            let pk = peercoin::PublicKey {
                inner: secp256k1::PublicKey::from_secret_key(&secp, &sk),
                compressed: true,
            };
            let sig = secp.sign_ecdsa(&msg, &sk);
            ecdsa_sigs.push(peercoin::ecdsa::Signature {
                sig,
            ...
            let keypair = peercoin::key::KeyPair::from_secret_key(&secp, &sk);
            let (x_only_pk, _parity) = peercoin::key::XOnlyPublicKey::from_keypair(&keypair);
            x_only_pks.push(x_only_pk);
            let schnorr_sig = secp.sign_schnorr_with_aux_rand(&msg, &keypair, &[0u8; 32]);
            let schnorr_sig = peercoin::taproot::Signature {
                sig: schnorr_sig,
                hash_ty: peercoin::sighash::TapSighashType::Default,
            };
    ...
    fn no_checks_ms(ms: &str) -> Miniscript<BitcoinKey, NoChecks> {
        ...
        let elem: Miniscript<peercoin::PublicKey, NoChecks> =
    ...
    fn x_only_no_checks_ms(ms: &str) -> Miniscript<BitcoinKey, NoChecks> {
        let elem: Miniscript<peercoin::key::XOnlyPublicKey, NoChecks> =
            Miniscript::from_str_ext(ms, &ExtParams::allow_all()).unwrap();
        elem.to_no_checks_ms()
    }
}
